...
Viteのマルチページ静的サイトとして実装する。ページはHTMLファイルとして配置し、共通のCSS/JSだけをViteでbundleする。Collectionは価格や購入導線を持たないブランドギャラリーとして扱う。GitHub Pages workflowは手動実行に限定し、公開はオーナー承認後に行う。
